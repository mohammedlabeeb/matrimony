<?php
		...
		$email = $_SESSION['email'];
                $sel="select * from register where (email='$email' or matri_id='$mid')";
	...
	$query = "SELECT COUNT(*) as num FROM $tbl_name where (find_in_set($con, part_country_living) > 0) AND (find_in_set($rel, part_religion) > 0) AND gender!='$gen'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages['num'];
	
	/* Setup vars for query. */
	$targetpage = "broader_matches.php"; 	//your file name  (the name of this file)
	...
	$SQL = mysql_query("select * from register_view where (find_in_set($con, part_country_living) > 0) AND (find_in_set($rel, part_religion) > 0) AND gender!='$gen'  LIMIT $start, $limit");
...
<div class="container">	
	...
	<?php include "page-part/menu.php";?><!-----------------------Menu part end------------------------->
 	<ol class="breadcrumb">
  		...
  		<li class="active">Broader Matches</li>
	</ol>
 	<div class="row">   
           <div class="col-xs-12 col-sm-9 col-sm-push-3 col-xs-push-0">
		<div class="panel panel-success">
        <div class="panel-heading">
     	<i class="glyphicon glyphicon-share"></i> The matche is based on country and religion. 
        </div>
        </div>    
          <div class="panel panel-warning">
                <div class="panel-heading">
                  <h3 class="panel-title">Broader Matches</h3>                
                </div>
