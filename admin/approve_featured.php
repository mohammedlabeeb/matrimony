  ...
  {     
	...
	if(isset($_POST['index_id']))
	{
	    ...
	    switch($ACTION)
	    {
			...
			case 'Featured':
				$SQL_STATEMENT =  "update  register set fstatus='Featured' where index_id in ".$index_id_val;
				
		$website =  $configObj->getConfigName();
		$webfriendlyname =  $configObj->getConfigFname();
		$from = $configObj->getConfigFrom();
	   
	 $result45 = mysql_query("SELECT * FROM email_templates where EMAIL_TEMPLATE_NAME = 'Featured Profile'");
...
setPageContext("member","approve-featured");
...
<div id="wrapper">
  ...
  <div id="container" class="cf">
    ...
    <div class="widecolumn alignleft">
      <div class="breadcum-wide"><a href="#" title="Members">Members</a> / <a href="#" title="Approve Paid to Featured Members List">Approve Paid to Featured Members List</a></div>
    ...
    <p class="submit-btn cf">
		...
		<input type="hidden" name="action"  id="filter_action" value="SEARCH" />
			...
			<?php
					...
					?><span style='float:right'><a href='approve_featured.php?option=clear_search&member_status=Paid'> <span title='Clear Filter'
	...
	<?php
		...
		$SQL_STATEMENT_COUNT = "select count(index_id) from register_view ".$where_member_status." ".$where_clause ." and fstatus=''";
		}
		else
		{
		
	    $SQL_STATEMENT_COUNT = "select count(index_id) from register_view".getWhereClauseForstatus($member_status). " and fstatus=''";
		}
	   ...
	   <div class="nodata-avail ">
            <h1>Approve Paid to Featured Members List</h1>
          </div>  	
 ...
 <a href="javascript:;" style="margin-left:25px;"  class="alignleft delete-btn" title="Featured" onclick="submitActionForm('Featured');">Featured</a>
		...
		if($search_case)
		{
				...
				if(empty($where_member_status))
					...
					$where_member_status .= " and ";
		$SQL_STATEMENT = "SELECT * FROM register_view ".$where_member_status." ".$where_clause." and fstatus='' ORDER BY index_id DESC LIMIT " . $lim_str;
		$SQL_STATEMENT_PAGINATION = "select count(index_id) as 'total_rows' from  register_view ".$where_member_status." ".$where_clause. " and fstatus=''";
		}
		else
		{
		$SQL_STATEMENT =  "SELECT * FROM register_view ".getWhereClauseForstatus($member_status)." and fstatus='' ORDER BY index_id DESC LIMIT ".$lim_str;
		$SQL_STATEMENT_PAGINATION = "select count(index_id) as 'total_rows' from register".getWhereClauseForstatus($member_status) ." and fstatus=''";	
		}
	...
	<form method="post" action="approve_featured.php" id="action_form">
	 <?php						
	...
	<!-- MEMBERSHIP PLAN-1 START -->
        ...
        <input type="hidden" name="email[]" value="<?php echo $DatabaseCo->dbRow->email; ?>" />
			...
			<label class="detail-desc2 cf"> <label class="title-label">Email:</label>
										...
										class="title-desc2"><?php $a=$DatabaseCo->dbRow->last_login;
...
<?php if($DatabaseCo->dbRow->status=="Active")
  ...
  <?php 
	    ...
	    echo getNewPagination('approve_featured.php','register_page_size','register','index_id',$page_size,$current_page,$SQL_STATEMENT_PAGINATION);
	  ?>	
     </div>
	 <?php
	  }
