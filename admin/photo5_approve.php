<?php
  ...
  {     
	...
	if(isset($_POST['index_id']))
	{
	    ...
	    {
		    case 'DELETE':		
				$SQL_STATEMENT =  "update  register set photo5='' where index_id in ".$index_val;	
			      break;
		    case 'APPROVED':
...
<script type="text/javascript">
	setPageContext("approve","photo5-appprove");
	$(document).ready(function ()
	   {
		$("#approove" ).button().click(function(){
		   window.location = "photo5_approve.php?photo_status=approved";
		 });
		$("#unapproove" ).button().click(function(){
		   window.location = "photo5_approve.php?photo_status=unapproved";
		 });
	       $("#all" ).button().click(function(){
		   window.location = "photo5_approve.php?photo_status=all";
		      });
	   });
</script>
</head>
...
<div id="wrapper">
  ...
  <div id="container" class="cf">
    ...
    <div class="widecolumn alignleft">
      <div class="breadcum-wide"><a href="#" title="Approval">Approval</a> / <a href="#" title="Photo 2 Approval List"> Photo 5 Approval List</a></div>
      <div class="listing-section">
        <div class="member-list cf">          
    <div class="approval alignleft">
	<input type="button"  title="Approove Photos"   id="approove"  value="Approved (<?php echo getRowCount("select count(index_id) from register where photo5!='' AND photo5_approve='APPROVED' AND status!='Inactive'",$DatabaseCo);?>)"/>+
	    <input type="button"  title="Unapproove Photos"  id="unapproove" value="Unapproved (<?php echo getRowCount("select count(index_id) from register where photo5 !='' AND photo5_approve='UNAPPROVED' AND status!='Inactive'",$DatabaseCo);?>)"/>=
	     <input type="button" title="All Photos"  id="all" value="All (<?php echo getRowCount("select count(index_id) from register WHERE photo5!='' AND status!='Inactive'",$DatabaseCo);?>)"/>	    
          </div>
        </div>
      </div>
      <?php
	...
	<?php
	  ...
	  $photo_approve_count = getRowCount("select count(index_id) from register".getWhereClauseForPhoto5($photo_status),$DatabaseCo);
	...
	<a href="javascript:;"  class="alignleft delete-btn" title="Approved" onclick="submitActionForm('UNAPPROVED');">Unapproved</a>
	  <?php
		...
		$SQL_STATEMENT =  "SELECT * FROM register ".getWhereClauseForPhoto5($photo_status)." ORDER By index_id DESC LIMIT ".$lim_str;
	  ?>
	...
	<form method="post" action="photo5_approve.php" id="action_form">
              <?php						
	...
	<tr class="<?php echo $cssClass;?>">
		    ...
		    <td title="<?php echo $DatabaseCo->dbRow->username;?>"><img src="../photos/watermark.php?image=<?php echo $DatabaseCo->dbRow->photo5; ?>&watermark=watermark.png" height="140px" width="180px" style="vertical-align:middle !important;"/></td>
            </tr>
		 <?php
		 $rowCount++;
	      }
          ...
          <?php 	
		$SQL_STATEMENT_PAGINATION = "select count(index_id) as 'total_rows' from  register".getWhereClauseForPhoto5($photo_status);		  
	echo getNewPagination('photo5_approve.php','photo_approve_page_size','register','index_id',$page_size,$current_page,$SQL_STATEMENT_PAGINATION);
	  ?>		  
        </div>
        <?php
	    }
