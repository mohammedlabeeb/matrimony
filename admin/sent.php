...
var refreshRequired = false;
 ...
 function getMessageDetail(mes_id)
 {
 	...
 	$.get("../admin/web-services/user/get_message_detail1.php?mes_id="+mes_id,
 	function(data){
	 ...
	 ?>       
      <div class="widecolumn-inner memership-detail">
        <?php
			...
			$SQL_STATEMENT_COUNT = "select count(mes_id) from messages where from_id='admin'";
	   ...
	   ?>
		...
		$SQL_STATEMENT =  "SELECT * FROM messages where from_id='admin' ORDER BY mes_id  DESC LIMIT ".$lim_str;
		$SQL_STATEMENT_PAGINATION = "select count(mes_id) as 'total_rows' from  messages where from_id='admin'";
?>
    
</div>
    <div class="cf">
    	<div class="my-property-outer">
              <form method="post" action="sent.php" id="action_form">
											...
											<div class="mycontent">
												<div class="cf email-section" style="display: block;">
													...
													<p class="email-detail alignleft">
														<a href="javascript:;" class="email-unread"
															onclick="getMessageDetail(<?php echo $DatabaseCo->dbRow->mes_id;?>);"> <?php echo $DatabaseCo->dbRow->subject;?></a> <span class="email-desc" style="width: 730px;"> 
															<?php echo getSuppressString($DatabaseCo->dbRow->message,200);?>
														</span> <span class="form-info">To:

															<?php echo $DatabaseCo->dbRow->to_id;?></span>
													</p>
													<p class="alignright">
													<?php $a=$DatabaseCo->dbRow->sent_date;
