 ...
 $(document).ready(function ()
   {
	...
	window.location = "country-list.php?country_status=approved";
      });
     $("#unapproove" ).button().click(function(){
	window.location = "country-list.php?country_status=unapproved";
      });
    $("#all" ).button().click(function(){
	window.location = "country-list.php?country_status=all";
      });
     ...
     $("#save" ).button().click(function(){
		...
		$.post(
		      "./web-services/location/add-country.php?action="+$("#update_action").val(),
		      $("#country-form").serialize(),
		      function(data){
   ...
   function edit_country(country_id,country_name,country_status)
      {
	 ...
	 $("#Country_Name").val(country_name);
	    ...
	    $("#country_status1").attr("checked","checked");
	 }else{	
	      $("#country_status2").attr("checked","checked");
	 }
	 ShowDialog(true);
	 $("#validationSummary").hide();
	 $("#Country_Name").focus();	      
...
?>
    <div class="widecolumn alignleft">
      <div class="breadcum-wide"><a href="#" title="User">Location</a> / <a href="#" title="Role"> Countries</a></div>
      <div class="listing-section">
        <div class="member-list cf">
          <a href="javascript:;" class="button" title="Add New User Role" id="add_country"><img src="img/bgi/add-icon.png" alt="Add"/>Add New Countries</a>
          <div class="approval alignleft">
          <input type="button" title="Approved Country List" id="approove" value="Approved (<?php echo getRowCount("select count(country_id) from country where status='APPROVED'",$DatabaseCo);?>)"/>
            +
            <input type="button" title="Unapproved Country List"  id="unapproove" value="Unapproved (<?php echo getRowCount("select count(country_id) from country where status='UNAPPROVED'",$DatabaseCo);?>)"/>
            =
            <input type="button" title="All Country List"   id="all" value="All (<?php echo getRowCount("select count(country_id) from country",$DatabaseCo);?>)"/>
      ...
      <div id="dialog" class="web_dialog">
        <p class="web_dialog_title" id="dialog_title">Add New Contry </p>
        ...
        <form action="" method="post" class="form-data" id="country-form">
          <p class="cf">
            <label class="popup-label">1. Country Name:</label>
            <input type="text" class="textBox"  name="country_name" id="Country_Name"/>
          </p>
          <p class="cf"> <label class="popup-label">2. Status:</label>
            <input type="radio"  value="APPROVED" name="country_visibility_status" id="country_status1"  />
            <span class="radio-btn-text">Active</span>
            <input type="radio"  value="UNAPPROVED" name="country_visibility_status" id="country_status2"  />
 ...
 <?php
    ...
    <?php
	    ...
	    $country_count = getRowCount("select count(country_id) from country".getWhereClauseForStatus($country_status),$DatabaseCo);
	    if($country_count>0){  
	   ?>
	    <div class="nodata-avail ">
            <h1><?php echo strtoupper($country_status); ?> Country List</h1>
          </div>   
  ...
  <a href="javascript:;"  class="alignleft delete-btn" title="Delete" onclick="submitActionForm('DELETE');">Delete</a>
	...
	<a href="javascript:;"  class="alignleft delete-btn" title="Approved" onclick="submitActionForm('UNAPPROVED');">Unapproved</a>          
	  <?php
		...
		$rec_not_found = true;
		$page_size = isset($_COOKIE["country_page_size"])?$_COOKIE["country_page_size"]:10;
		$lim_str = getLimitForSqlState($current_page,$page_size);
		
		$SQL_STATEMENT =  "SELECT * FROM country ".getWhereClauseForStatus($country_status)." ORDER BY country_id DESC  LIMIT ".$lim_str;
?>
        ...
        <div class="table-desc cf">
          <table width="100%" cellpadding="0" cellspacing="0" border="1" class="table-data">
            <tr>
              ...
              <th width="23%">Country ID</th>
              <th width="57%">Country Name</th>
            </tr>
            <form method="post" action="country-list.php" id="action_form">
              <?php						
		...
		while($DatabaseCo->dbRow = mysql_fetch_object($DatabaseCo->dbResult))
		{
        	...
        	?>
              <tr class="<?php echo $cssClass;?>">
                <td><input type="checkbox"  class="table-checkbox" name="country_id[]" value="<?php  echo $DatabaseCo->dbRow->country_id;?>" /></td>
                <td><a class="edit-btn margin-none" href="#" title="Edit" id="edit_country" onclick="edit_country('<?php  echo $DatabaseCo->dbRow->country_id;?>','<?php  echo $DatabaseCo->dbRow->country_name;?>','<?php  echo $DatabaseCo->dbRow->status;?>');">Edit</a></td>
                <td><?php
			$likeDisLikeCss = "";
			if($DatabaseCo->dbRow->status=="APPROVED")
		      ...
		      ?>
                ...
                <td ><?php echo $DatabaseCo->dbRow->country_id;?></td>
	  ...
	  ?>
          ...
          <?php 	
		$SQL_STATEMENT_PAGINATION = "select count(country_id) as 'total_rows' from  country".getWhereClauseForStatus($country_status);		  
	echo getNewPagination('country-list.php','country_page_size','country','country_id',$page_size,$current_page,$SQL_STATEMENT_PAGINATION);
?>		  
        </div>
		<!-- copy from here -->
		        <?php
        ...
        <div class="table-desc cf">
          <div class="nodata-avail ">
            <h1>There are no data for <?php echo strtoupper($country_status); ?> Country. Please add data.</h1>
          <br/>
