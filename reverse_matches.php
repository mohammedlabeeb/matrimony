...
$age;
	...
	$adjacents = 3;
	 ...
	 $query = "SELECT COUNT(*) as num FROM $tbl_name where (find_in_set($con, part_country_living) > 0) AND part_height<='$hei' AND (find_in_set($rel, part_religion) > 0)  $f AND gender!='$gen' AND (find_in_set($caste, part_caste) > 0) AND part_frm_age<=$age";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages['num'];
	
	/* Setup vars for query. */
	$targetpage = "reverse_match.php"; 	//your file name  (the name of this file)
	...
	/* Get data. */
	$SQL = mysql_query("select * from register_view where (find_in_set($con, part_country_living) > 0) AND part_height<='$hei' AND (find_in_set($rel, part_religion) > 0)  $f AND gender!='$gen' AND (find_in_set($caste, part_caste) > 0) AND part_frm_age<=$age LIMIT $start, $limit");
 	...
 	<div class="row">   
      <div class="col-sm-3">
        ...
        <div class="panel-heading">
     	<i class="glyphicon glyphicon-share"></i> This match is based on other's partner preference like education, height, religion, country etc.. 
        </div>
        </div>    
          <div class="panel panel-warning">
                <div class="panel-heading">
                  <h3 class="panel-title">Reverse Matches</h3>                
                </div>
