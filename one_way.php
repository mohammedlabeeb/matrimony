...
$g=$row['gender'];
	...
	$query = "SELECT COUNT(*) as num FROM $tbl_name where country_id IN('$cou') AND religion IN ('$rel') AND caste IN ('$caste') AND gender!='$g' AND ((
			(
	...
	$targetpage = "one_way.php"; 	//your file name  (the name of this file)
	...
	/* Get data. */
	 ...
	 $SQL=mysql_query("select * from register_view where country_id IN('$cou') AND religion IN ('$rel') AND caste IN ('$caste') AND gender!='$g' AND ((
			(
			date_format( now( ) , '%Y' ) - date_format( birthdate, '%Y' )
			) - ( date_format( now( ) , '00-%m-%d' ) < date_format( birthdate, '00-%m-%d' ) )
			)
			BETWEEN '$e'
			AND '$f') LIMIT $start, $limit");		
 ...
 <div class="wrapper gradient">  
	...
	<article style="margin-top:-50px;">
        ...
        <div class="panel-heading">
     		<i class="glyphicon glyphicon-share"></i> This match is based on your partner preference like religion,caste, country,age etc..
        </div>
        </div>
          <div class="panel panel-warning">
                <div class="panel-heading">
                  <h3 class="panel-title">I am Looking for</h3>                
                </div>
